...
먼저 단순 이동 평균(이하 SMA)은 며칠 동안의 종가를 모두 합한 후 평균을 낸 것입니다. SMA(10)는 10일 동안 종가를 평균 낸 것이고, SMA(30)은 30일 동안 종가를 평균 낸 것입니다. 기간이 길면 길수록 가격변동이 부드럽게 보여주며, 추세를 보기 쉽습니다. 
...
RSI는 과매수, 과매도를 판단하기 위한 지표입니다. 과매수는 주식 상승으로 인해 많이 매수된 상태를 말하고, 과매도는 주식 하락으로 인해 많이 매도된 상태를 말합니다. 과매수 상태를 천정으로 유추하고 과매도를 바닥으로 유추한다. 이 RSI지수는 최소값이 0이고, 최대값이 100입니다. 직관적으로 추세의 강도를 의미하며, 높을수록 과매수 상태이고, 낮을수록 과매도 상태이다. 계산법은 아래와 같습니다.
...
RSI에서도 여러가지 파라미터가 있을 수 있는데, "일정기관"이 7일, 14일 등으로 설정할 수 있고, 과매도 및 과매수 임계값을 70, 30이 아니라, 80, 20으로 설정할 수 있습니다. 또한 매매 시점을 임계값을 초과할 때 바로 보는 것이 아니라 유지되다가 다시 탈피할 때라던지 등 여러가지 시점으로 볼 수 있습니다.
